...
void updateCamera(int dist, int steps){
	float stepSize = 320 / steps;
	int row = (int)(240 - 120*(1+(tan(atan(dist / CAMERA_HEIGHT) - CAMERA_ANGLE)/tan(LENS_VIEW_ANGLE)))); // I can explain this math if anyone wants to know what it does, I drew a picture and everything
	row = max(min(row, 239),0);
	int maxBright = 0; // integer for tracking 
	int minBright = 0;
	int pixel[steps];
...
int main(){
	...
	for(int i = 0; i < 30; i++){
		take_picture();
		update_screen();
		for(float ii = 3.5; ii < 15.5; ii++){
			updateCamera((int)ii,160);
		}
		update_screen();	
		sleep1(0,500000);
	}
